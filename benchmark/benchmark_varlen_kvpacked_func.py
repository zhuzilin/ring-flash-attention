...
def benchmark(
    f, use_double_cu_seqlens, num_iter=100, forward_only=True, log=True, profile=False
):
    ...
    def wrapper(use_double_cu_seqlens: bool, i: int):
        if use_double_cu_seqlens:
            return f(
                q,
                kv,
    ...
    if forward_only:
        with torch.no_grad():
            for i in range(num_iter):
                _ = wrapper(use_double_cu_seqlens, i)
    else:
        for i in range(num_iter):
            q.grad = None
            kv.grad = None
            out = wrapper(use_double_cu_seqlens, i)
            out.backward(dout)
            if profile:
                profiler.step()
